...
#include "crazyflieReturnToBase.h"
...
CCrazyflieReturnToBase::CCrazyflieReturnToBase()
    : m_pcDistance(NULL),
      m_pcPropellers(NULL),
      m_pcPos(NULL),
      m_pcP2PSend(NULL),
...
void CCrazyflieReturnToBase::Init(TConfigurationNode &t_node) {
    try {
        /*
    ...
    droneReturnToBase = {ROTATING, ROTATING, 0, 0, {0, 0, 0}, {0, 0, 1}, wallPositions};
}

/****************************************/
/****************************************/

void CCrazyflieReturnToBase::ControlStep() {
    void *pointer = static_cast<void *>(sensors);

    if(ReturnToBase(&droneReturnToBase, &commander, pointer)) {
        LOG << GetId() << " : At the base" << std::endl;
    }
}
...
void CCrazyflieReturnToBase::Reset() {}
 ...
 */
REGISTER_CONTROLLER(CCrazyflieReturnToBase, "crazyflie_return_to_base")
